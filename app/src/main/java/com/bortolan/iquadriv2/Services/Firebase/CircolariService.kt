...
class CircolariService : JobService() {
    override fun onStartJob(job: JobParameters?): Boolean {

        val preferences = PreferenceManager.getDefaultSharedPreferences(applicationContext)
        if (Methods.isNetworkAvailable(applicationContext)) {
            DownloadCircolari(preferences) { list ->
                if (list == null || list.isEmpty()) return@DownloadCircolari

                Log.d("CircolariService", list[0].title)
